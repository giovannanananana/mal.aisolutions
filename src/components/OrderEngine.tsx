...
const OrderEngine = () => {
  ...
  const handleSubmit = async () => {
    ...
    const subject = encodeURIComponent(`New Project Inquiry - ${services.find(s => s.id === selectedService)?.label}`);
  ...
  const handleCardHover = (e: React.MouseEvent<HTMLDivElement>, isEntering: boolean) => {
    gsap.to(e.currentTarget, {
      ...
      boxShadow: isEntering ? '0 0 30px hsl(var(--crimson) / 0.2)' : '0 0 0px transparent',
      duration: 0.3,
      ease: 'power2.out',
    });
  };
  ...
  return (
    <section ref={sectionRef} id="order" className="py-24 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div
          ref={formRef}
          className="order-engine-card rounded-2xl p-8 md:p-10"
          style={{
        ...
        >
          ...
          {/* Step Content */}
          <div ref={stepContentRef} className="min-h-[300px]">
            {/* Step 1: Services */}
            {currentStep === 1 && (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {services.map((service) => (
                    <div
                      ...
                      className={`service-option cursor-pointer p-6 rounded-xl text-center transition-all duration-300 will-change-transform ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {budgets.map((budget) => (
                    <div
                      ...
                      onMouseEnter={(e) => handleCardHover(e, true)}
                      onMouseLeave={(e) => handleCardHover(e, false)}
                      className={`budget-option cursor-pointer p-5 rounded-xl text-center transition-all duration-300 will-change-transform ${
